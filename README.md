...
Включить: `/roast on` — бот будет изредка (~5% сообщений, не чаще раза в 10 мин) отвечать на сообщения группы через GPT-4o-mini.
...
| `OPENAI_API_KEY` | нет | Ключ OpenAI API (нужен для `/roast`) |
