...
def build_tag_line(participants: list[tuple[int, str]]) -> str:
    if not participants:
        return ""
    tags = " ".join(_user_link(uid, name) for uid, name in participants)
...
def build_gather_text(session: Session) -> str:
    ...
    tag_part = f"\n\n{session.tag_line}" if session.tag_line else ""
    return (
        f"{header}\n\n"
